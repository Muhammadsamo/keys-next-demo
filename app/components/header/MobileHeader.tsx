  ...
  return (
    <div className="mobile-header" ref={el => { mobHeaderRef.current = el; }}>
      ...
      <div className={`mobile-header-nav${open ? ' open' : ''}`}>
        <nav>
          <ul>
            <li><button className="mob-link" type="button" onClick={() => { gsap.to(window, { duration: 1, scrollTo: { y: '#about', autoKill: true } }); setOpen(false); }}>About</button></li>
            <li><button className="mob-link" type="button" onClick={() => { gsap.to(window, { duration: 1, scrollTo: { y: '#vision', autoKill: true } }); setOpen(false); }}>Our Vision</button></li>
            <li><button className="mob-link" type="button" onClick={() => gsap.to(window, { duration: 1, scrollTo: { y: `#home`, offsetY: 0, autoKill: true } })}>Latest Event</button></li>
            <li><button className="mob-link" type="button" onClick={() => { gsap.to(window, { duration: 1, scrollTo: { y: '#services', autoKill: true } }); setOpen(false); }}>Services</button></li>
          </ul>
        </nav>
